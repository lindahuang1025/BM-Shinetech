...
// import FormRegister from './UserEdit';
const { TextArea } = Input;

const CommentList = (
  { currentUser, comments, onDeleted } // 滚动加载
) => (
  // <InfiniteScroll
  // initialLoad={false}
  // pageStart={0}
...
const User = (props) => {
  ...
  const newFuturePrompt = () => {
    // message.info('喔嚯，这个功能还在上线中哦！', 5);
    //  message.info(currentUser.UserId, 5);
    history.push({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      message.success('biu~ 🚀 ');
      getUserCommentsList();
      handleChange(null); // rocketRef.current.style.left = '90%';
    } catch (error) {
      ...
      <div>
        {userCommentsList.length > 0 && (
          <CommentList
            ...
            onDeleted={deletedCommentByAdmin}
          />
        )}
        <Comment
          avatar={
            ...
            <>
              <Form.Item>
                <TextArea
                  ...
                  value={editorValue}
                  placeholder="自由发言啦，大家可以在这里留下想说的话:想看的书或观后感、生活工作的心得、当成树洞等等，最后最后，各位看官，来点软件优化建议吧（PS：是匿名显示滴，社恐患者不要慌哈）"
                />
              </Form.Item>
              <Form.Item>
